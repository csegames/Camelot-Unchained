...
export function parseEmoji(text: string, opts: OptionsState, next: Parser): [string, JSX.Element] {
  if (!opts.parsing.emoji) {
    return [text, null];
  }
  const regex = /(?<= |^):[a-zA-Z0-9]+:(?= |^)|[3]*(?<= |^)[;:8][-']*[()@oO#$*pPD/|><](?= |$)|\([6aAhH]\)|-_-zzZ|\(-_-\)zzZ|[oO]\.[oO]/g;
  ...
  const after = text.substr(match.index).replace(emoji, '');
  const emojiName = emojiNameFromText(emoji);
  return [
    before,
    (
      <>
        <span key={genID()} className={`emoji emoji--${emojiName}`} />
