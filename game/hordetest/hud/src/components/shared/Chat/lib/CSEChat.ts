...
import {
  ...
  createEventEmitter,
  EventEmitter,
} from '@csegames/library/lib/_baseGame';

const chat = require('@csegames/library/lib/chat/chat_pb.js');
import '@csegames/library/lib/chat/chatProtoTypes';
    ...
    switch (msg.getType()) {
      ...
      case ChatClientUnionMessage_MessageTypes.ROOMINFO:
        ...
        this.eventEmitter.emit(
          ...
          ],
          {
            ...
            roles: info.getRolesList().map((role: any) => ({
              name: role.getName(),
      ...
      case ChatClientUnionMessage_MessageTypes.DIRECTORY:
        ...
        this.eventEmitter.emit(
          ...
          {
            rooms: directory.getRoomsList().map((info: any) => ({
              roomID: info.getRoomid(),
              name: info.getName(),
              category: info.getCategory(),
            })),
